...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=%q", order.GetFilename()))
		if _, err := io.Copy(w, obj); err != nil {
			slog.Error("Unable to stream object to client", slog.String("error", err.Error()))
			// cannot write header now if body started; log and return
		}
	}
}
...
func PostOrder(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = s3.PutObject(r.Context(), storage.OrdersBucket, dbOrder.GetFilename(), reader, int64(reader.Len()), minio.PutObjectOptions{ContentType: "text/markdown"})
		if err != nil {
